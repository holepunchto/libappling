...
on_unlink(fs_unlink_t *req, int status) {
  ...
  e = appling_bootstrap(loop, js, &bootstrap_req, key, &link, "test/fixtures/bootstrap/no-platform-v1", on_bootstrap);
  assert(e == 0);
}
...
main() {
  ...
  e = fs_unlink(loop, &unlink_req, "test/fixtures/bootstrap/no-platform-v1/current", on_unlink);
  assert(e == 0);
